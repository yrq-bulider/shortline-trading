...
> 版本：v3.1 · 2026-06-10（v3 → v3.1: 加 §3.6 实时协作日志 AGENT_LOG.md）
...
### 3.6 实时协作日志 AGENT_LOG.md（v3.1 新增）

**根目录有一个 `AGENT_LOG.md`，本地白板，不进 git（.gitignore 已配 `/AGENT_LOG.md`）。**

- **起步必做**：`cat AGENT_LOG.md` 或 Read，看 § in-flight 锁段 + 最近 10 条历史事件
- **改文件前**：在 § in-flight 锁加一行 `<时间> · <agent> · 改 <文件>`
- **改完后**：从锁删自己那行 + 在 § 历史事件追一条 `<时间> · <agent> · <文件> · <动作> · <commit-sha 或 WIP>`
- **业务约束**：写在文件 § 业务约束 段（用户拍板的硬规则，agent 不能违反）

**Why**：§5 死命令"不同时编辑同一文件"是口头约定，无锁机制 → `cc25eaf` 事故（Codex `git add -A` 顺手 commit Claude 工作区改动）。本日志 = 轻量锁 + 事件流。
...
| v3.1 | 2026-06-10 | 加 §3.6 实时协作日志 AGENT_LOG.md（弥补 §5 无锁机制，cc25eaf 事故 follow-up） |
